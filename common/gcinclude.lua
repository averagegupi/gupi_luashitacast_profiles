...
gcinclude.settings = {
	...
	DTGearHPP = 1; -- set HPP to have your DT set to come on
	PetDTGearHPP = 1; -- set pet HPP to have your PetDT set to come on
...
function gcinclude.SetVariables()
	...
	if (player.MainJob == 'RDM') or (player.MainJob == 'BLM') or (player.MainJob == 'SCH') or (player.MainJob == 'DRK') then
	...
	if (player.MainJob == 'MNK') then
		...
		gcdisplay.CreateToggle('mdt', false);
		gcdisplay.CreateToggle('acc', false);
		gcdisplay.CreateToggle('eyy', false);
	end
...
function gcinclude.HandleCommands(args)
	...
	if (player.MainJob == 'RDM') or (player.MainJob == 'BLM') or (player.MainJob == 'NIN') or (player.MainJob == 'DRK') then
	...
	if (player.MainJob == 'NIN') then
		...
		gcdisplay.AdvanceToggle('silence');
		toggle = 'silence';
		status = gcdisplay.GetToggle('silence');
		end
		if (args[1] == 'fudo') then
		gcdisplay.AdvanceToggle('fudo');
		toggle = 'fudo';
	...
	if (player.MainJob == 'MNK') then
		...
		if (args[1] == 'eyy') then
			gcdisplay.AdvanceToggle('eyy');
			toggle = 'eyy';
			status = gcdisplay.GetToggle('eyy');
		end
	end
	if (player.MainJob == 'PUP') then
...
function gcinclude.DoShadows(spell) -- 1000% credit to zach2good for this function, copy and paste (mostly) from his ashita discord post
	if spell.Name == 'Utsusemi: Ichi' then
		local delay = 2.4
		if gData.GetBuffCount(446) == 1 then
